...
function extractToolTitleFromInput(input: unknown): string | null {
  if (typeof input === "string") {
    ...
    try {
      ...
      return extractToolTitleFromInput(parsed);
    } catch {
      return null;
    }
  }
  ...
  const toolTitle = input.tool_title;
  if (typeof toolTitle !== "string") {
    return null;
  }

  const trimmed = toolTitle.trim();
  return trimmed.length > 0 ? trimmed : null;
}

const formatToolHeaderLabel = (type: string, input: unknown) => {
  const prefix = "tool-";
  const toolName = type.startsWith(prefix) ? type.slice(prefix.length) : type;
  const toolTitle = extractToolTitleFromInput(input) ?? toolName;
  return `${toolTitle}`;
};
  ...
  >
    <div className="flex min-w-0 flex-1 items-center gap-2">
      ...
      <span className="truncate font-medium text-sm">
        {formatToolHeaderLabel(type, input)}
      </span>
    </div>
